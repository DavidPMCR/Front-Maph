...
const createPatientScreen = ({ route }) => {
  ...
  const handleDeletePatient = async () => {
    try {
      ...
      const response = await axios.delete(`http://192.168.1.98:3001/patient/${formData.id_cedula}`);
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        fetchPatients(); // Recargar lista de pacientes
      } else {
...
const styles = StyleSheet.create({
  ...
  input: { height: 50, borderColor: '#ccc', borderWidth: 1, borderRadius: 5, padding: 10, marginBottom: 10 },
  pickerContainer: { borderWidth: 1, borderColor: '#ccc', borderRadius: 5, marginBottom: 20 },
  buttonContainer: { flexDirection: 'row', justifyContent: 'space-between' },
  editButton: { flex: 1, marginRight: 5, backgroundColor: '#007bff', padding: 10, borderRadius: 5 },
  createButton: { flex: 1, marginLeft: 5, backgroundColor: '#28a745', padding: 10, borderRadius: 5 },
  buttonText: { color: '#fff', textAlign: 'center' },
  modalContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' },
  modalContent: { backgroundColor: '#fff', padding: 20, borderRadius: 5, width: '90%' },
